...
<div class="profile-panel" id="profilePanel">
    ...
    <form id="profileForm" action="update_profile.php" method="POST" enctype="multipart/form-data">
        <div class="profile-field">
            <label>Name: </label>
            <span id="nameText"><?= htmlspecialchars($customer['name']) ?></span>
            <input type="text" name="name" id="nameInput" value="<?= htmlspecialchars($customer['name']) ?>" style="display:none;">
            <span class="edit-btn" onclick="toggleEdit('name')">Edit</span>
        </div>

        <div class="profile-field">
            <label>Login: </label>
            <span id="loginText"><?= htmlspecialchars($customer['login']) ?></span>
            <input type="text" name="login" id="loginInput" value="<?= htmlspecialchars($customer['login']) ?>" style="display:none;">
            <span class="edit-btn" onclick="toggleEdit('login')">Edit</span>
        </div>

        <div class="profile-field">
            <label>Email: </label>
            <span id="emailText"><?= htmlspecialchars($customer['email']) ?></span>
            <input type="text" name="email" id="emailInput" value="<?= htmlspecialchars($customer['email']) ?>" style="display:none;">
            <span class="edit-btn" onclick="toggleEdit('email')">Edit</span>
        </div>

        <div class="profile-field">
            <label>Profile Picture:</label><br>
            <?php if (!empty($customer['profile_picture'])): ?>
                <img src="data:image/jpeg;base64,<?= base64_encode($customer['profile_picture']) ?>" alt="Profile Picture">
            <?php else: ?>
